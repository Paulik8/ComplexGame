...
Moving::Moving(QGraphicsScene & scene, QGraphicsItem *p1, QGraphicsItem *p2, QGraphicsItem *ball, QObject *parent) :
    ...
    P2Shift( 0 )
{
    Scene.setSceneRect(0, 0, 350, 320);//задание начальных размеров игрового окна
    Scene.addItem(P1);
    Scene.addItem(P2);
    Scene.addItem(Ball);
...
void Moving::time()
{
    ...
    if ( ( nX + Ball->boundingRect().left() < Scene.sceneRect().left() ) || ( nX + Ball->boundingRect().right() > Scene.sceneRect().right() ) )//условие смены направления мяча по Ох
    {
        BallShift.rx() *= -1;
    }
