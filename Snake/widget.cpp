...
Widget::Widget(QWidget* parent) :
  ...
  for (int i = 0; i < ROWS; i++) {
    for (int j = 0; j < COLUMNS; j++) {
      // взаимодействие пользователя с ячейками
  ...
  // добавление кнопок и счетчика очков
  ...
  connect(&snake,
          SIGNAL(drawSnake(QVector<QPoint>, QVector<QPoint>)),
          this,
          SLOT(drawSnakeSlot(QVector<QPoint>, QVector<QPoint>)));
...
// передвижение змейки на одну ячейку
